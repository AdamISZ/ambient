...
async fn test_snicker_end_to_end() -> Result<()> {
    ...
    let (mut alice_mgr, alice_mnemonic) =
        Manager::generate(&alice_name, "regtest", current_height).await?;
    ...
    let (mut bob_mgr, bob_mnemonic) =
        Manager::generate(&bob_name, "regtest", current_height).await?;
    ...
    // Debug: Check how many proposals are in Alice's database
    let stored_proposals = alice_mgr.snicker.get_all_snicker_proposals().await?;
    println!("   Debug: Alice's database has {} stored proposals", stored_proposals.len());
