    ...
    C::Target: RpcApi,
{
    ...
    pub fn next_block(&mut self) -> Result<Option<BlockEvent<Block>>, bitcoincore_rpc::Error> {
        if let Some((checkpoint, hash)) = poll(self, move |hash, _| Ok(*hash))? {
            let height = checkpoint.height();
            println!("Starting next block with this height: {}", height);

            // Below 980_000: fetch only the header, fake an empty block.
            if height < 980_000 {
                let header: Header = self.client.get_block_header(&checkpoint.hash())?;
                let fake_block = Block {
                    header,
                    txdata: Vec::<Transaction>::new(), // empty body
                };
                ...
                let block = self.client.get_block(&checkpoint.hash())?;

                // Stop tracking unconfirmed txs that got confirmed in this block.
