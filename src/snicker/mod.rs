...
mod tests {
    ...
    // Helper to create a mock LocalOutput for testing
    ...
    // Returns (PSBT, tweaked_script) where tweaked_script is the script for output 0
    fn create_test_psbt(
        ...
        let receiver_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x01; 32])
            .unwrap();
        let receiver_key = bdk_wallet::bitcoin::secp256k1::XOnlyPublicKey::from_keypair(
            &bdk_wallet::bitcoin::secp256k1::Keypair::from_secret_key(&secp, &receiver_seckey)
        ).0;

        let proposer_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x02; 32])
            .unwrap();
        let proposer_key = bdk_wallet::bitcoin::secp256k1::XOnlyPublicKey::from_keypair(
            &bdk_wallet::bitcoin::secp256k1::Keypair::from_secret_key(&secp, &proposer_seckey)
        ).0;

        let tweaked_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x03; 32])
            .unwrap();
        let tweaked_key = bdk_wallet::bitcoin::secp256k1::XOnlyPublicKey::from_keypair(
            &bdk_wallet::bitcoin::secp256k1::Keypair::from_secret_key(&secp, &tweaked_seckey)
        ).0;

        let change_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x04; 32])
            .unwrap();
        let change_key = bdk_wallet::bitcoin::secp256k1::XOnlyPublicKey::from_keypair(
            &bdk_wallet::bitcoin::secp256k1::Keypair::from_secret_key(&secp, &change_seckey)
        ).0;
    ...
    fn test_validate_amounts_correct_delta() {
        ...
        // Receiver has 50k, pays 1k delta, gets 49k output
        let (psbt, tweaked_script) = create_test_psbt(
            receiver_outpoint,
            50_000,  // receiver input
            proposer_outpoint,
            100_000, // proposer input
            vec![49_000, 49_000, 50_000], // equal outputs (49k each) + change (50k)
        );
        ...
        let tweak_info = TweakInfo {
            ...
            proposer_pubkey: bdk_wallet::bitcoin::secp256k1::PublicKey::from_slice(
                &[0x02; 33]
            ).unwrap(),
        };
        ...
        // Delta is 1000 sats, should be accepted in range (0, 5000)
        let result = snicker.validate_amounts(
            &psbt,
            &tweak_info,
            &our_utxos,
            (0, 5000), // acceptable delta range
        );
    ...
    fn test_validate_amounts_rejects_missing_tweaked_output() {
        ...
        let different_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x05; 32])
            .unwrap();
        let different_key = bdk_wallet::bitcoin::secp256k1::XOnlyPublicKey::from_keypair(
            &bdk_wallet::bitcoin::secp256k1::Keypair::from_secret_key(&secp, &different_seckey)
        ).0;
    ...
    fn test_validate_inputs_all_p2tr() {
        ...
        let proposer_pubkey = bdk_wallet::bitcoin::secp256k1::PublicKey::from_slice(&proposer_pubkey_bytes)
            .unwrap();
    ...
    fn test_validate_inputs_rejects_mixed_types() {
        ...
        let receiver_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x01; 32])
            .unwrap();
        let receiver_full_pubkey = bdk_wallet::bitcoin::secp256k1::PublicKey::from_secret_key(&secp, &receiver_seckey);
        ...
        let proposer_full_pubkey = bdk_wallet::bitcoin::secp256k1::PublicKey::from_secret_key(&secp, &proposer_seckey);
        ...
        let actual_proposer_pubkey = bdk_wallet::bitcoin::secp256k1::PublicKey::from_slice(&actual_proposer_bytes)
            .unwrap();
    ...
    fn test_validate_inputs_rejects_key_mismatch() {
        ...
        let wrong_seckey = bdk_wallet::bitcoin::secp256k1::SecretKey::from_slice(&[0x06; 32])
            .unwrap();
        let wrong_proposer_pubkey = bdk_wallet::bitcoin::secp256k1::PublicKey::from_secret_key(
            &secp,
            &wrong_seckey
        );
