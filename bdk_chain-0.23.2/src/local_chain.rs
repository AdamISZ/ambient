...
impl LocalChain {
    ...
    pub fn from_blocks(blocks: BTreeMap<u32, BlockHash>) -> Result<Self, MissingGenesisError> {
        ...
        println!("Starting to build LocalChain from_blocks");
        for block in &blocks {
            match tip {
                Some(curr) => {
                    tip = Some(
        ...
        println!("Finished the block loop for LocalChain from_blocks");
        Ok(Self {
    ...
    pub fn apply_update(&mut self, update: CheckPoint) -> Result<ChangeSet, CannotConnectError> {
        println!("About to merge chains for update up to height {}", update.height());
        let (new_tip, changeset) = merge_chains(self.tip.clone(), update)?;
        println!("Finished merging chains, new tip at height {}", new_tip.height());
        self.tip = new_tip;
    ...
    pub fn apply_header_connected_to(
        ...
        println!("Applying header at height {}", height);
        let this = BlockId {
            height,
        ...
        println!("About to apply update for header at height {}", height);
...
fn merge_chains(
    ...
    println!("About to apply changeset to original tip");
